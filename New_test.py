...
def fetch_comments(ticker, start_date, end_date, max_retries=5, max_tweets=5000):
    ...
    while True:
        ...
        for message in data['messages']:
            ...
            if tweet_count >= max_tweets:
                print(f"Tweet limit of {max_tweets} reached for {ticker}. Stopping scraping.")
                print(f"Last day reached: {created_at.strftime('%Y-%m-%d')}")
                print(f"Number of tweets on last day: {day_tweet_count}")
                return comments, True
